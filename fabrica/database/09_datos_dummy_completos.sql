...
-- Nota: PART_REVIEW usa COMMENT_TEXT (schema). Si JPA creó la tabla con BODY,
--       cambia COMMENT_TEXT por BODY en el INSERT de PART_REVIEW.
-- =============================================================================
...
BEGIN
  FOR u_rec IN (SELECT USER_ID uid FROM APP_USER WHERE EMAIL LIKE 'cliente%@test.com' OR EMAIL LIKE 'empresa%@corp.com') LOOP
    ...
    FOR ord_num IN 1..4 LOOP
      ...
      SELECT order_header_seq.NEXTVAL INTO v_order_id FROM DUAL;
      ...
      VALUES (order_status_history_seq.NEXTVAL, v_order_id,
...
BEGIN
  FOR i IN 1..40 LOOP
    ...
    BEGIN
      INSERT INTO PART_REVIEW (REVIEW_ID, PART_ID, USER_ID, PARENT_REVIEW_ID, RATING, COMMENT_TEXT, CREATED_AT)
